...
class Flight:
    ...
    def __init__(self, rocket, environment,
        ...
        self.step = timeStep if isinstance(timeStep, list) else 2*[timeStep]
        ...
        while self.solver.successful() and (self.solver.y[0]**2 +
            ...
            self.solver.integrate(self.solver.t + self.step[0])
        ...
        # # Out of rail until first event phase
        # # Intialize macro time counter
        # self.t = self.tFinal
        # # Determine macro discretization points based on sensor sampling rates
        # points = []
        # for pc in self.rocket.parachutes:
        #     points.append(np.arange(self.t, self.maxTime, 1/pc.samplingRate))
        # discretizationPoints = np.unique(points)
        # timeSteps = np.diff(discretizationPoints)
        # # Iterate over each macro discretization point
        # for i in range(length(discretizationPoints))  
        #     # Check current state (if flying freely or under parachute)
        #     solver = scipy.integrate.solve_ivp(fun, t_span, y0, method='RK45', t_eval=None, dense_output=False, events=None, vectorized=False, **options)
        #     # Update current time
        #     self.t = discretizationPoints[i+1]
        ...
        while self.solver.successful() and (self.solver.y[2] < 0 and
                                            self.solver.t < self.maxTime):
            self.solver.integrate(self.solver.t + self.step[0])
            self.solution.append([self.solver.t, *self.solver.y])
        while self.solver.successful() and (self.solver.y[5] >= 20 and
                                            self.solver.t < self.maxTime):
            self.solver.integrate(self.solver.t + self.step[1])
            self.solution.append([self.solver.t, *self.solver.y])
