...
class Function:
    ...
    def __init__(self,
        ...
        Parameters
        ----------
        source : function, int, float, ndarray, string
    ...
    def setExtrapolation(self, method='zero'):
    ...
    def getValue(self, *args):
        ...
        args : int, float, list
            ...
            one being an int, float or list.
    ...
    def plot1D(self, lower=None, upper=None, density=1000, forceData=False, forcePoints=False):
        'Plots function on specified grid.'
        ...
        x = np.linspace(lower, upper, density)
    ...
    def plot2D(self, lower=None, upper=None, density=[20, 20], forceData=True,
               dispType='surface'):
        'Plots function on specified grid.'
        # Prepare plot
        figure = plt.figure()
        ...
        # Create nodes to evaluate function
        x = np.linspace(lower[0], upper[0], density[0])
        y = np.linspace(lower[1], upper[1], density[1])
    ...
    def __interpolatePolynomial__(self):
        'Calculates polynomail coefficients that fit the data exactly.'
    ...
    def __interpolateSpline__(self):
        'Calculates splines coefficients that fit the data exactly.'
        # Get x and y values for all supplied points
        x = self.source[:, 0]
        y = self.source[:, 1]
        mdim = len(x)
    ...
    def __interpolateAkima__(self):
        'Calculates akima Spline coefficients that fit the data exactly'
        # Get x and y values for all supplied points
        x = self.source[:, 0]
        y = self.source[:, 1]
    ...
    def integral(self, a, b, numerical=False):
        """ Evaluate a definite integral of the function in the interval
        from a to b.
        ...
        numerical : bool
            ...
            If False, try to calculate using interpolation information. If
...
class Environment:
        ...
        if isinstance(windData, str):
            if windData[-4:] == '.csv':
                windData = np.loadtxt(windData, delimiter=',',
                                      dtype=np.float64)
                windSpeed = windData[:, (0, 1)]
                windDirection = windData[:, (0, 2)]
                self.windDataSource = 'csvFile'
            elif windData[-3:] == '.nc' or windData[-4:] == '.grb':
                windData = netCDF4.Dataset(windData)
                self.processNetCDFFile(windData)
                self.windDataSource = 'weatherFile'
                processed = True
            else:
                raise TypeError('Only .csv, .nc and .grib file formats '
                                'are supported.')
        elif isinstance(windData, (tuple, list, np.ndarray)):
            if len(windData) == 2 and isinstance(windData[0], (float, int)):
                self.windSpeed = Function(lambda height: windData[0], 'Height (m)',
                                          'Wind Speed (m/s)', 'spline', 'constant')
                self.windDirection = Function(lambda height: windData[1], 'Height (m)',
                                              'Wind Heading Direction (degrees)',
                                              'spline', 'constant')
                self.windVelocityX = Function(lambda height: windData[0] *
                                              np.cos(windData[1]*np.pi/180),
                                              'Height (m)', 'Wind Velocity X (m/s)')
                self.windVelocityY = Function(lambda height: -windData[0] *
                                              np.sin(windData[1]*np.pi/180),
                                              'Height (m)', 'Wind Velocity Y (m/s)')
                self.windDataSource = 'array'
                processed = True
            elif len(windData) == 2:
                windSpeed = windData[0]
                windDirection = windData[1]
                self.windDataSource = 'array'
            else:
                raise TypeError('Only arrays of length 2 are accepeted. '
                                'The first argument should be wind speed data '
                                'and the second one should be wind direction.')

        if processed is False:
            # Process wind data
            self.windSpeed = Function(windSpeed, 'Height (m)',
                                      'Wind Speed (m/s)',
                                      'spline', 'constant')
            self.windDirection = Function(windDirection, 'Height (m)',
                                          'Wind Heading Direction (degrees)',
                                          'spline', 'constant')
            self.windVelocityX = Function(lambda height: self.windSpeed(height) *
                                          -np.cos((self.windDirection(height)-90)*np.pi/180),
                                          'Height (m)', 'Wind Velocity X (m/s)')
            self.windVelocityY = Function(lambda height: -self.windSpeed(height) *
                                          -np.sin((self.windDirection(height)-90)*np.pi/180),
                                          'Height (m)', 'Wind Velocity y (m/s)')
            if not callable(windSpeed) and not callable(windDirection):
                # Set discrete
                initZ = windSpeed[0, 0]
                finalZ = windSpeed[-1, 0]
                numOfPoints = len(windSpeed)
                self.windVelocityX.setDiscrete(initZ, finalZ, numOfPoints)
                self.windVelocityY.setDiscrete(initZ, finalZ, numOfPoints)
    ...
    def setDate(self, date):
        self.date = datetime(*date)
        if self.windDataSource == 'weatherFile':
