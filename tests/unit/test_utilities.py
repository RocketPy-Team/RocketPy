...
@pytest.mark.skip(reason="legacy tests")  # it is not wokring
def test_create_dispersion_dictionary():
    ...
    test_dict = {}
    with open("tests/fixtures/monte_carlo/Valetudo_inputs.csv", mode='r') as csvfile:
        reader = csv.reader(csvfile, delimiter=';')
        next(reader)  # Skip header
        for row in reader:
            key, value, std_dev = row[1].strip(), row[2].strip(), row[3].strip()
            if key:
                if std_dev:
                    try:
                        test_dict[key] = (float(value), float(std_dev))
                    except ValueError:
                        test_dict[key] = (value, std_dev)
                else:
                    try:
                        test_dict[key] = float(value)
                    except ValueError:
                        try:
                            test_dict[key] = eval(value)
                        except SyntaxError:
                            test_dict[key] = value
