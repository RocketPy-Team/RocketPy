...
for i in range(1, len(vert_vel_kalt), 1):
    acc = (vert_vel_kalt[i] - vert_vel_kalt[i - 1]) / (time_kalt[i] - time_kalt[i - 1])
...
print("Real data: {:.2f} m/s".format(max(vert_vel_kalt)))
print("RocketPy - Real data: {:.2f} m/s".format(max(velocity_rcp) - max(vert_vel_kalt)))
print()
