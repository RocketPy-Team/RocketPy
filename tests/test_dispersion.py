...
#     assert np.isclose(np.mean(all_margins), 0.5, rtol=0.15)
#     assert np.isclose(np.std(all_margins), 0, rtol=0.15)


# def test_monte_carlo_simulate(calisto_dispersion):
#     mc = calisto_dispersion.simulate(number_of_simulations=10, append=False)

#     assert mc.num_of_loaded_sims == 10


# def test_monte_carlo_prints(calisto_dispersion):

#     calisto_dispersion.prints.all()


# def test_monte_carlo_plots(calisto_dispersion):
#     assert calisto_dispersion.plots.ellipses() is None
#     assert calisto_dispersion.plots.all() is None
