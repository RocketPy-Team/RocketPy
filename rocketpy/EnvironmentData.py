...
class EnvironmentData(BaseModel):
    ...
    railLength : tuple, list
        ...
        rail. Should be given as a tuple or a list. The tuple can have two
        or three items. The tuple's first item is the nominal value, the
        second item is the standard deviation, and the optional third item
        is string with the name of the distribuition type used in the dispersion.
        If nothing is passed for the third item, normal distribuition is used.
        If passed as a list, each item of the list will be ramdonly chosen
        in each iteration of the dispersion.
    date : list, optional
    ...
    elevation : tuple, list, optional
        ...
        specified. Default value is 0. Should be given as a tuple or a list. The tuple
        can have two or three items. The tuple's first item is the nominal value, the
        second item is the standard deviation, and the optional third item is string
        with the name of the distribuition type used in the dispersion. If nothing is
        passed for the third item, normal distribuition is used. If passed as a list,
        each item of the list will be ramdonly chosen in each iteration of the dispersion.
    gravity : tuple, list, optional
        Surface gravitational acceleration. Positive values point the acceleration down.
        Default value is 9.80665. Should be given as a tuple or a list. The tuple can
        have two or three items. The tuple's first item is the nominal value, the second
        item is the standard deviation, and the optional third item is string with the
        name of the distribuition type used in the dispersion. If nothing is passed for
        the third item, normal distribuition is used. If passed as a list, each item of
        the list will be ramdonly chosen in each iteration of the dispersion.
    latitude : tuple, list, optional
        ...
        or if Open-Elevation will be used to compute elevation. Should be given as a tuple
        or a list. The tuple can have two or three items. The tuple's first item is the
        nominal value, the second item is the standard deviation, and the optional third
        item is string with the name of the distribuition type used in the dispersion.
        If nothing is passed for the third item, normal distribuition is used. If passed
        as a list, each item of the list will be ramdonly chosen in each iteration of
        the dispersion.
    longitude : tuple, list, optional
        ...
        model or if Open-Elevation will be used to compute elevation. Should be given as a
        tuple or a list. The tuple can have two or three items. The tuple's first item is
        the nominal value, the second item is the standard deviation, and the optional
        third item is string with the name of the distribuition type used in the
        dispersion. If nothing is passed for the third item, normal distribuition is used.
        If passed as a list, each item of the list will be ramdonly chosen in each
    ...
    railLength: Any
    date: List[Union[Tuple[int, int, int, int], None]] = [None]
    elevation: Any = (0, 0)
    gravity: Any = (9.80665, 0)
    latitude: Any = (0, 0)
    longitude: Any = (0, 0)
    datum: List[StrictStr] = [
        "SIRGAS2000"
    ]  # TODO: can add even more restriction to be just the acceptable strings
    timeZone: List[StrictStr] = ["UTC"]
...
class EnvironmentDataByEnvironment(BaseModel):
    """Class that holds that about an environment to be used in the Dispersion class.
