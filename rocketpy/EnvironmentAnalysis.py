...
class EnvironmentAnalysis:
    ...
    def animate_wind_rose(self):
        ...
        def update(i):
            print(i)
            ax.bar(windDir[hours[i]], windSpeed[hours[i]], normed=True, opening=0.8, edgecolor="white")
            ax.set_title(f'Windrose of an average day. Hour: {hours[i]}')

        ani = FuncAnimation(fig, update, range(len(hours)), init_func=init)
        plt.show()
