...
def generate(ork_file, nb_file, eng_file, open_rocket_instance):
    ...
    time_vect = [
        float(datapoint.text.split(",")[0]) for datapoint in bs.find_all("datapoint")
    ]
    starting_pos = 0
    ...
    thrust_vect = [
        float(datapoint.text.split(",")[28]) for datapoint in bs.find_all("datapoint")
    ][starting_pos:final_pos]
    cg_location_vect = [
        float(datapoint.text.split(",")[24]) for datapoint in bs.find_all("datapoint")
    ][starting_pos:final_pos]

    altitude_vect = [
        float(datapoint.text.split(",")[1]) for datapoint in bs.find_all("datapoint")
    ][starting_pos:final_pos]
    apogee_index = np.argmax(altitude_vect)

    propelant_mass_vect = [
        float(datapoint.text.split(",")[20]) for datapoint in bs.find_all("datapoint")
    ][starting_pos:final_pos]
    mass = [
        float(datapoint.text.split(",")[19]) for datapoint in bs.find_all("datapoint")
    ][starting_pos:final_pos]
    drag_coefficient = [
        float(datapoint.text.split(",")[30]) for datapoint in bs.find_all("datapoint")
    ][starting_pos:final_pos][0:apogee_index]
    mach_number = [
        float(datapoint.text.split(",")[26]) for datapoint in bs.find_all("datapoint")
    ][starting_pos:final_pos][0:apogee_index]

    normalize_propelant_mass_vect = np.array(propelant_mass_vect)
    normalize_propelant_mass_vect = (
        normalize_propelant_mass_vect
        - normalize_propelant_mass_vect[np.argmin(normalize_propelant_mass_vect)]
    )
    ...
    empty_rocket_cm = cg_location_vect[
        burnout_position
    ]  # calculate_rocket_cg(ork.getRocket(), rocket_mass)
    ...
    longitudinal_moment_of_inertia = [
        float(datapoint.text.split(",")[21]) for datapoint in bs.find_all("datapoint")
    ][burnout_position]
    rotational_moment_of_inertia = [
        float(datapoint.text.split(",")[22]) for datapoint in bs.find_all("datapoint")
    ][burnout_position]
    ...
    for idx, main_chute in enumerate(
        ...
        main_cds = (
            "auto"
            if main_chute.find("cd").text == "auto"
    ...
    flight_parameters.update(
        {
            ...
            "railLength": 12,
            "dragCoefficientSourcePath": drag_coefficient_source_path,
            "inclination": 84,
            "heading": 133,
            "latitude": 39.3897,
...
def calculate_rocket_cg(ork, rocket_mass):
    ...
    for section in ork.getChild(0).getChildren():
        cg_rocket += (
            section.getSectionMass() * (top_position + section.getCG().x) / rocket_mass
        )
...
def generate_motor_code(
    ...
    thrust_vect = np.array(
        [time_vect[0:burnout_position], thrust_vect[0:burnout_position]]
    ).T
...
with orhelper.OpenRocketInstance() as instance:
    ...
    ork_file = (
        "./Trajectory Simulations/13 - Team13_Open RocketProject_v1.00/rocket.ork"
    )
    eng_file = ""
    nb_file = "./docs/notebooks/getting_started.ipynb"
    generate(ork_file, nb_file, eng_file, instance)
