...
__author__ = "Giovani Hidalgo Ceotto, Guilherme Fernandes Alves, João Lemes Gribel Soares, Patrick Sampaio"
...
class Flight:
    ...
    def animate_trajectory(self, Object3dFile, start=0, stop=None, timeStep=0.1):
        ...
        world = Box(
            [self.x(start), self.y(start), self.apogee],
            max(self.x[:, 1]) * 2,
            max(self.y[:, 1]) * 2,
            -self.apogee,
        ).wireframe()
        ...
        rocket.pos(self.x(start), self.y(start), 0).addTrail(n=len(self.x[:, 1]))
        ...
        for t in np.arange(start, stop, timeStep):
            ...
            rocket.pos(self.x(t), self.y(t), self.z(t) - self.env.elevation).rotateX(
                self.e1(t) / k
            ).rotateY(self.e2(t) / k).rotateZ(self.e3(t) / k)
    ...
    def animate_rotate(self, Object3dFile, start=0, stop=None, timeStep=0.1):
        ...
        for t in np.arange(start, stop, timeStep):
            angle = np.arccos(2 * self.e0(t) ** 2 - 1)
            k = np.sin(angle / 2)
            rocket.pos(self.x(start), self.y(start), 0).rotateX(self.e1(t) / k).rotateY(
                self.e2(t) / k
            ).rotateZ(self.e3(t) / k)
