...
class Flight:
    ...
    def uDot(self, t, u, postProcessing=False):
        ...
        # Transform coordinate system from Local (xyz) to Equatorial (XYZ)
        X, Y, Z, R_xyz_XYZ = self.transformToEquatorial(x, y, z)
        # Calculate g_{XYZ}(XYZ)
        gX, gY, gZ = self.gravity(X, Y, Z)
        # Transform back
        gx, gy, gz = self.transformToLocal(gX, gY, gZ, X, Y, Z, vx, vy, vz, R_xyz_XYZ)
        # Add to derivative
        ax += gx
        ay += gy
        az += gz
    ...
    def transformToEquatorial(x, y, z):
        """ Transforms position vector from Local to Equatorial coordinate system.
        Local system:
        ...
        b = r_e - r_e*f # Earth's polar radius, km
        ...
        ut1 = self.env.date
        year, month, day = ut1.years, ut1.months, ut1.days
        hour, minute, second = ut1.hours, ut1.minutes, ut1.seconds
        if month < 3:
            year  -= 1
            month += 12
        JD  = int(365.25 * year) \
        ...
        return X, Y, Z, R_xyz_XYZ

    def gravity(X, Y, Z):
        """ Calculates the gravitational acceleration vector based on position vector in the Equatorial frame
        using the geopotential zonal coefficients up to the 6th order.


        Parameters
        ----------
        X, Y, Z : floats
            Position vector in Earth coordinates, in kms.
        
        Returns
        -------
        gX, gY, gZ : floats
            Acceleration vector in Earth coordinate system.
        """
        ...
        return gX, gY, gZ # the acceleration



    def transformToLocal(gX, gY, gZ, X, Y, Z, vx, vy, vz, R_xyz_XYZ):
        """ Converts vector from the Equatorial coordinate system to the Local based system.
        Local system:
            Origin at the launch rail,
            z coordinate perpendicular to the ground,
            x coordinate due east,
            y coordinate due north.
        Equatorial coordinate system:
            Origin at the centre of the Earth,
            The fundamental plane in the plane of the Earth's equator.
            The primary direction (the x axis) toward the vernal equinox, that is, the place where the Sun
        crosses the celestial equator in a northward direction in its annual apparent circuit around the ecliptic,
            A right-handed convention, specifying a y axis 90° to the east in the fundamental plane, and 
            z axis along the north polar axis.
        ...
        # Earth angular velocity in the Local frame
        ...
        return g_xyz[0], g_xyz[1], g_xyz[2]
